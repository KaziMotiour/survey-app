...
const SurveyReview = () => {
  ...
  // get Single Surevey with review info
  useEffect(() => {
    getSurveyDetails(id);
  }, [id]);
  ...
  const getSurveyDetails = async (id) => {
    ...
    try {
      const sureveyDetail = await axios.get(
          ...
          <Form.Group as={Row} className='mb-2' controlId='formPlaintextEmail'>
            ...
            <Col sm='10'>
              <Form.Control
                ...
                value={surveyDetails && surveyDetails.title}
                disabled
              />
            </Col>
          </Form.Group>
          ...
          >
            ...
            <Col sm='10'>
              <Form.Control
                ...
                name='title'
                placeholder='Survey title'
                value={surveyDetails && surveyDetails.timer}
                disabled
              />
            </Col>
          </Form.Group>
                      ...
                      <Col sm='9'>
                        <Accordion defaultActiveKey='1'>
                          <Accordion.Item eventKey='0'>
                            <Accordion.Header>
                              You Got {question.text_answer} Answer
                            </Accordion.Header>
                            {question.question_from &&
                              question.question_from.map((answer, index) => (
                                <>
                                  <Accordion.Body>
                                    <p>
                                      Answer {index + 1} :{" "}
                                      <span style={{ color: "#0c92ee" }}>
                                        {answer.text_answer}
                                      </span>
                                    </p>
                                  </Accordion.Body>
                        ...
                        {question.question.map((option, index) => (
                          ...
                          >
                            ...
                            <Col sm='2'>
                              <Form.Control
                                style={{
                                  backgroundColor: "#2995bc",
                                  color: "white",
                                }}
                                name='qts'
                                question_type='text'
                                placeholder='Add new question answer'
                                value={option.vot_count + " vot"}
                                disabled
                              />
                            </Col>
                          </Form.Group>
